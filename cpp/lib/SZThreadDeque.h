#ifndef SZLIBRARY_SZTHREADDEQUE_H
#define SZLIBRARY_SZTHREADDEQUE_H
...
public:
    ...
    bool pop_fonrt(value_type &element, int64_t timeout = -1)
    {
        ...
        if (deque_.empty())
        {
            ...
            if (timeout < 0)
            {
                cond_.wait(locker, [&] { return !deque_.empty(); });
            }
            else if (timeout > 0)
            {
                cond_.wait_for(locker, std::chrono::milliseconds(timeout), [&] { return !deque_.empty(); });
            }
        }
...
#endif  // SZLIBRARY_SZTHREADDEQUE_H
