...
export namespace szheap {
    export class Heap<T> {
        // 比较函数，用于构建最大堆或者最小堆
        private compare_: szcommon.conditonCompare<T> = null;
         ...
         */
        public constructor(compareFunc: szcommon.conditonCompare<T> = szcommon.greaterCompare) {
            this.compare_ = compareFunc;
            this.data_ = new Array<T>(szdefine.DEFAULT_CAPACITY).fill(undefined);
            this.size_ = 0;
        }
        ...
        public set capacity(c: number) {
            ...
            if (c < this.data_.length) {
                ...
                this.data_.push(...new Array<T>(c - this.data_.length).fill(undefined));
            }
         ...
         */
        private resize() {
            this.data_.push(...new Array<T>(Math.ceil(this.data_.length * (szdefine.DEFAULT_GROWTH_FACTOR - 1))).fill(undefined));
        }
    }
} // namespace sz
