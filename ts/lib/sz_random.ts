 ...
 * 默认范围[0, 1)
 */
export class CarrySubtractGenerator {
    private readonly m: number;
    private readonly a: number;
...
export class DiscreteDistributionGenerator {
    // 随机数引擎
    private gen_: LinearCongruentialGenerator;
    ...
    constructor(seed: number, probabilities: number[]) {
        ...
        this.gen_ = new LinearCongruentialGenerator(seed);
 ...
 * 使生成列表之和为指定值
 */
export class IntegerListDistributionGenerator {
    // 最大调整次数
     ...
     * 洗牌打乱索引数组
     */
    private suffleListIndex() {
        for (let i = 0; i < this.indexList_.length; i++) {
            let j = Math.floor(this.gen_.next() * (i + 1));
            [this.indexList_[i], this.indexList_[j]] = [this.indexList_[j], this.indexList_[i]];
        }
    }
